# STM32F051R8T6
Projects using microcontroller STM32F051R8T6






Lab2 
Design a seven-segment display driver
to interface a seven-segment display and display information from the embedded system
using port interrupts
using SysTick to debounce a switch
Design a four-digit, seven-segment interface
to learn how to create a software interface
to learn how to use time-multiplexing
using the SN74HC595N 8-bit shift register IC
